...
const App = (props) => {
  ...
  const Average = (props) => {
    ...
    const resp = sumall == 0 ? "No feedback given" : "Average " + ((props.good - props.bad) / sumall ).toFixed(1).toString()
    
    return (<h2> {resp} </h2>
            )
  }

  const Positive1 = (props) => {
    const sumall = (props.good + props.neutral + props.bad)
    const resp = sumall == 0 ? "" : "Positive" + (good / sumall * 100).toFixed(2).toString() + "%"
  ...
  <div>
   ...
   <h1>Statistics </h1>
    ...
    <Positive1 good={good}  bad={bad} neutral={neutral} />    
    </div>
  )
}
